    ...
    try:
        with open(TOKEN_EXP_FILE_PATH, "r") as token_exp_file:
            ...
            if not token_exp_str:
                logger.warning(f"Token expiration file {TOKEN_EXP_FILE_PATH} is empty. Assuming expired.")
                return False
        ...
        if is_expired:
            ...
            logger.debug(f"webapi_token is valid. Expires at {datetime.fromtimestamp(token_exp_timestamp)}")
        return not is_expired
    except FileNotFoundError:
        logger.warning(f"Token expiration file not found at {TOKEN_EXP_FILE_PATH}. Assuming expired.")
        return False
    except ValueError as e:
        logger.error(f"Invalid timestamp in {TOKEN_EXP_FILE_PATH}: '{token_exp_str}'. Error: {e}. Assuming expired.")
        return False
    except Exception as e:
        logger.error(f"Error checking token expiration from {TOKEN_EXP_FILE_PATH}: {e}. Assuming expired.")
        return False
...
def get_token() -> str:
    ...
    try:
        ...
        logger.warning(f"Token file not found at {TOKEN_FILE_PATH}. Returning empty token.")
        return ""
    except Exception as e:
