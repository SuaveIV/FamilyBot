...
async def handle_websocket(websocket, path):
    ...
    try:
        ...
        async for message in websocket:
            logger.info(f"Received message from {websocket.remote_address}: {message[:50]}...") # Log first 50 chars of message
            ...
            try:
                ...
                padded_token_part = message.split('.')[1].replace('-', '+').replace('_', '/')
                padded_token_part += '=' * (-len(padded_token_part) % 4)

                key_info = json.loads(base64.b64decode(padded_token_part).decode('utf-8'))
                ...
                logger.info(f"Expiration timestamp {exp_timestamp} written to file: {TOKEN_EXP_FILE_PATH}.")
            except (IndexError, json.JSONDecodeError, base64.binascii.Error) as e:
                logger.error(f"Error decoding or parsing token: {e}. Raw message start: {message[:100]}")
            except IOError as e:
                logger.error(f"Error writing token expiration to file {TOKEN_EXP_FILE_PATH}: {e}")
        ...
        logger.critical(f"Unexpected error in handle_websocket for {websocket.remote_address}: {e}", exc_info=True)
    finally:
