...
def convert_player_summaries(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"type": "batch", "data": converted_users}
...
def main():
    ...
    # Additional options
    parser.add_argument(
        "--steam-id", "-s", help="Steam ID (required for some API types)"
    )
    ...
    try:
        ...
        print(
            f"🎉 Conversion completed! Found {len(converted_data.get('data', []))} records",
            file=sys.stderr,
        )
        return 0
