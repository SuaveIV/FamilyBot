...
class JSONDatabaseImporter:
    ...
    def __init__(self, dry_run: bool = False, verbose: bool = False):
        ...
        self.stats = {"processed": 0, "added": 0, "skipped": 0, "errors": 0}
    ...
    def import_family_member(
        ...
        if self.dry_run:
            self.log_action(
                f"[DRY RUN] Would add family member: {friendly_name} (Steam {steam_id})"
            )
        else:
            try:
                conn = get_db_connection()
    ...
    def import_json_data(self, json_data: Any) -> bool:
        ...
        if isinstance(json_data, dict):
            ...
            self.log_action(
                f"List complete: {success_count}/{len(json_data)} records processed successfully"
            )
            return success_count > 0
