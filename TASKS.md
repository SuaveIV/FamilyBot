...
- [ ] Update `last_update` only _after_ the `asyncio.sleep` call.

### P4-04 — Fix `make_request_with_retry` jitter placement

**Priority:** 🟠 High
**Description:** `SteamAPIManager.make_request_with_retry` and `admin_commands.py:make_request_with_retry` should only apply jitter on _retry_ attempts.
