...
1. **Replace `requests` with `httpx.AsyncClient`**
    ...
    - Update all _Storefront/Price_ API calls in plugins (files using `requests.get`)
...
- Write to the database every 50-100 successful items _during_ the collection phase.
